...
function drawScene(){ //main drawScene function
	 clear(); //clear canvas
	 ctx.beginPath(); //custom shape beginPath
	 ctx.fillStyle ='rgba(5, 110, 110, 0.5)';
	 ctx.moveTo(circles[0].x, circles[0].y);
	 for (var i=0; i<circles.length; i++) {
		ctx.lineTo(circles[i].x, circles[i].y);
	}

	ctx.closePath(); // custom shape and 
	ctx.fill(); //fill custom shape
	ctx.lineWidth = 5;
	ctx.strokeStype ='rgba(0,0, 255, 0.5)';
...
$(function(){
	...
	var circleCount = 7; //we will draw 7 circles randomly
	...
	$('scene').mousemove(function(e) { //binding mousemove event for dragging selected circle
		...
		if(selectedCircle != undefined){
			...
			circles[selectedCircle] = new Circle(mouseX, mouseY, radius); //changing positions of selected circle
		}
		
		hoveredCircle = undefined;
		for(var i =0; i < circles.length;i++){ //checking through all circle - are mouse down inside circle or not
			var circleX = circles[i].x;
			var circleY = circles[i].y;
			var radius = circles[i].radius;
			if(Math.pow(mouseX-circleX,2) + Math.pow(mouseY - circleY, 2) < Math.pow(radius,2)) {
